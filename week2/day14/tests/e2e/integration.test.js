...
process.env.JWT_SECRET = process.env.JWT_SECRET || 'day14-test-jwt-secret-key-string';
...
describe('End-to-End Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await User.deleteMany({ email: { $in: ['integration@test.com', 'newuser@test.com'] } });
    ...
    testUser = new User({
      ...
      password: 'SecurePassword123!', // Validates password rules (uppercase, lowercase, number, special char)
      role: 'user'
    });
    await testUser.save();
    ...
    // Clean up any orders associated with this user ID
  ...
  afterAll(async () => {
    ...
    // Disconnect database client cleanly to let Jest exit
    ...
    test('User can register, login, browse products, and place order', async () => {
      ...
      const registerResponse = await request(app)
        ...
        .send({
          ...
          password: 'SecurePassword123!', // must pass strong validation
          role: 'user'
        });
      ...
      const newUserId = loginResponse.body.data.user.id;
      ...
      expect(analyticsResponse.body.data.totalOrders).toBeGreaterThan(0);

      // Clean up orders created by the new user
      await Order.deleteMany({ userId: newUserId });
    });
  });
